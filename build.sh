...
if [ -z $1 ] || [ -z $2 ] ; then
    echo "usage: $(basename $0) <mc_version> <launcher_version>"
    echo "requirements"
    echo "    openjdk-8-jre-headless"
    echo "    git"
    echo"     launcher install jar"
    exit 1
fi
...
repo_name="forge_minecraft_jdk8_2"
...
launcher_version=$2
launcher_installer_jar="forge-${app_version}-${launcher_version}-installer.jar"
if [ ! -e "${launcher_installer_jar}" ] ; then
  echo "Please download ${launcher_installer_jar}."
  echo "Exiting"
  exit 1
fi
...
mkdir -p ${rootfs}/opt/mc/mods_jar
...
#cp "${install_launcher_jar}" "${rootfs}/opt/mc/jar/"
...
docker build "${project_dir}" --no-cache --build-arg RCONPWD="${rconpwd}" --build-arg APP_VERSION="${app_version}" --build-arg LAUNCHER_INSTALLER_JAR="${launcher_installer_jar}" --build-arg LAUNCHER_VERSION="${launcher_version}"  -t "${local_repo_tag}"
